...
function defaultPlan(){
  return [
    ...
    {id:18,day:'C',muscle:'רגליים',name:'פשיטת ברכיים במכונה',reps:'10-12',sets:3,weights:[20,20,20],coach:'דרופ סט אופציונלי — לא ממהרים להעלות משקל',rest:90},
    {id:19,day:'C',muscle:'רגליים',name:'כפיפת ברכיים במכונה',reps:'10-12',sets:3,weights:[35,40,40],coach:'דרופ סט אופציונלי',rest:90},
    {id:20,day:'C',muscle:'רגליים',name:'תאומים',reps:'10-12',sets:3,weights:[40,40,30],coach:'דרופ סט אופציונלי — משענת 4',rest:90},
    {id:21,day:'C',muscle:'כתפיים',name:'לחיצת כתפיים משקולות יד',reps:'6-8, 8-10, 10-12',sets:3,weights:[25,25,20],coach:'',rest:90},
    {id:22,day:'C',muscle:'כתפיים',name:'הרחקת כתפיים לצדדים',reps:'10,15,20',sets:3,weights:[27,36,36],coach:'',rest:90},
    {id:23,day:'C',muscle:'כתפיים',name:'הרמת שכמות',reps:'12-15',sets:3,weights:[20,55,55],coach:'',rest:90},
    {id:24,day:'D',muscle:'גב',name:'משיכה באחיזה משולש סופינציה',reps:'6-8',sets:3,weights:[85,85,85],coach:'',rest:120},
    {id:25,day:'D',muscle:'גב',name:'חתירה רחבה בישיבה',reps:'12',sets:3,weights:[41,50,18],coach:'',rest:90},
    {id:26,day:'D',muscle:'חזה',name:'אהמר עליון',reps:'10-12',sets:3,weights:[35,35,30],coach:'',rest:90},
    {id:27,day:'D',muscle:'חזה',name:'פרפר לבחירה',reps:'12-15',sets:3,weights:[64,68,68],coach:'',rest:90},
    {id:28,day:'D',muscle:'יד אחורית',name:'פשיטת מרפק מאחורי הראש — לחיצה צרפתית',reps:'8-12',sets:3,weights:[0,0,0],coach:'',rest:90},
    {id:29,day:'D',muscle:'יד אחורית',name:'פשיטה יד-יד',reps:'6-8, 8-10, 10-12',sets:3,weights:[24,24,31],coach:'',rest:90},
    {id:30,day:'D',muscle:'יד קידמית',name:'W הפוך ולהפוך',reps:'8-10, 10-12, 12-15',sets:3,weights:[20,20,20],coach:'דרופ סט בסט אחרון — 2 משקלי עבודה',rest:90},
    {id:31,day:'D',muscle:'יד קידמית',name:'כפיפה בשיפוע על ספסל',reps:'12',sets:3,weights:[0,0,0],coach:'דרופ סט בסט אחרון — 2 משקלי עבודה',rest:90},
    {id:32,day:'D',muscle:'חזה',name:'תרגיל פיזיו',reps:'10-12',sets:3,weights:[0,0,0],coach:'',rest:90},
  ];
}
...
function renderExercises(){
  ...
  c.innerHTML = exes.map((ex,idx)=>{
    ...
    return `<div class="ex-card ${done?'done':''}" id="ec-${ex.id}">
...
function toggleSet(exId,sIdx,rest){
  const k=exId+'_'+sIdx;
  if(completedSets[k]) delete completedSets[k];
  else { completedSets[k]=true; startRest(rest); }
  saveSets(); renderExercises(); renderTodayCard();
}
...
function finishWorkout(){
  ...
  history.unshift({
    date:new Date().toISOString(), day:currentDay,
    dayName:DAY_INFO[currentDay].name, desc:DAY_INFO[currentDay].desc,
    exercises:exes.map((ex) => ({name:ex.name,muscle:ex.muscle,weights:ex.weights,reps:ex.reps,allDone:isExDone(ex)}))
  });
  ...
  renderExercises(); renderTodayCard(); renderHistory(); renderStats();
}

function startRest(secs){
  clearInterval(restInterval); restTotal=secs; restRemaining=secs;
  document.getElementById('rest-overlay').classList.add('open');
  updateRestUI();
  restInterval=setInterval(()=>{ restRemaining--; updateRestUI();
    if(restRemaining<=0){ clearInterval(restInterval); document.getElementById('rest-overlay').classList.remove('open'); showToast('⏰ הגיע הזמן!'); }
  },1000);
}
function updateRestUI(){
  document.getElementById('rest-num').textContent=restRemaining;
  const deg=(restTotal-restRemaining)/restTotal*360;
  document.getElementById('rest-circle').style.background=`conic-gradient(var(--accent) ${deg}deg, var(--border) ${deg}deg)`;
}
...
function startWorkout(){
  ...
  wmExercises.forEach((ex)=>{
    wmSetData[ex.id]=Array.from({length:ex.sets},(_,i)=>({weight:ex.weights[i]||ex.weights[0]||0,reps:'',done:!!saved[ex.id+'_'+i]}));
  });
...
function renderWM(){
  ...
  const setsHtml = sets.map((s,i)=>{
    ...
    return `<div class="wm-set-row ${cls}" id="wmr-${ex.id}-${i}">
      <div class="wm-snum">${s.done?'✓':(i+1)}</div>
      <div class="wm-sinfo">
        <div class="wm-starget">סט ${i+1} • יעד: ${ex.reps}</div>
        <div class="wm-shint">${s.done?'הושלם ✓':'לחץ ✓ לסיום הסט'}</div>
      </div>
      <div class="wm-sinputs">
        <div class="wm-igroup"><label>משקל ק"ג</label>
          <input type="number" step="2.5" value="${s.weight||''}" ${s.done?'disabled':''} oninput="wmUpdate(${ex.id},${i},'weight',this.value)"/>
        </div>
        <div class="wm-igroup"><label>חזרות</label>
          <input type="number" value="${s.reps||''}" placeholder="?" ${s.done?'disabled':''} oninput="wmUpdate(${ex.id},${i},'reps',this.value)"/>
        </div>
        <button class="wm-scheck" onclick="wmDoneSet(${ex.id},${i},${ex.rest})">${s.done?'✓':'✓'}</button>
      </div>
    </div>`;
  }).join('');

  document.getElementById('wm-body').innerHTML = `
    <div class="wm-ex-muscle">${ex.muscle}</div>
    <div class="wm-ex-name">${ex.name}</div>
    ${ex.coach?`<div class="wm-coach">🏅 ${ex.coach}</div>`:''}
    <div class="wm-sets">${setsHtml}</div>
  `;

  const isLast = wmIdx===total-1;
  const allDone = sets.every((s) => s.done);
  document.getElementById('wm-footer').innerHTML = `
    ${wmIdx>0?`<button class="wm-btn wm-prev" onclick="wmGo(-1)">⟵ הקודם</button>`:'<div></div>'}
    ${isLast&&allDone
      ?`<button class="wm-btn wm-next wm-end" onclick="wmFinish()">🎉 סיים אימון!</button>`
      :`<button class="wm-btn wm-next" onclick="wmGo(1)">${isLast?'סיים':'הבא ⟶'}</button>`}
  `;
}

function wmUpdate(exId,i,field,val){ wmSetData[exId][i][field] = field==='weight'?parseFloat(val)||0:val; }

function wmDoneSet(exId,i,rest){
  const s=wmSetData[exId][i];
  s.done=!s.done;
  const k=exId+'_'+i;
  if(s.done){ completedSets[k]=true; saveSets(); startRest(rest); }
  else { delete completedSets[k]; saveSets(); }
  renderWM(); renderExercises(); renderTodayCard();
}
  ...
  list.innerHTML=history.slice(0,30).map((h,i)=>{
    ...
    return `<div class="hist-card">
      <div class="hist-hdr" onclick="toggleHist(${i})">
        <div><div class="hist-date">${date} — ${h.dayName}</div><div class="hist-type">${h.desc}</div></div>
        <div class="hist-badge">${done}/${h.exercises.length} ✓</div>
      </div>
      <div class="hist-body" id="hb-${i}">
        ${h.exercises.map((e)=>`<div class="hist-row"><span>${e.name}</span><span style="color:var(--muted)">${e.weights&&e.weights[0]>0?e.weights[0]+' ק"ג':'BW'} × ${e.reps}</span>${e.allDone?'<span style="color:var(--green)">✓</span>':''}</div>`).join('')}
      </div>
    </div>`;
  }).join('');
}
...
function renderStats(){
  ...
  document.getElementById('stats-grid').innerHTML=`
    <div class="stat-card"><div class="stat-val">${total}</div><div class="stat-lbl">סה"כ אימונים</div></div>
    <div class="stat-card"><div class="stat-val">${week}</div><div class="stat-lbl">השבוע</div></div>
    <div class="stat-card"><div class="stat-val">${history.filter((h,i,a)=>a.findIndex((x)=>x.day===h.day)===i).length}</div><div class="stat-lbl">סוגי אימונים</div></div>
    <div class="stat-card"><div class="stat-val">${maxW}</div><div class="stat-lbl">שיא משקל ק"ג</div></div>
  `;
}
...
function renderPlan(){
  document.getElementById('plan-container').innerHTML = DAYS_ORDER.map((d)=>{
    ...
    return `<div class="sec-title">${DAY_INFO[d].emoji} ${DAY_INFO[d].name} — ${DAY_INFO[d].desc}</div>
      ${exes.map((ex)=>`<div class="plan-row" onclick="openEdit(${ex.id})">
        <div class="plan-muscle">${ex.muscle}</div>
        <div class="plan-name">${ex.name}</div>
        <div class="plan-meta">${ex.sets}×${ex.reps}<br>${ex.weights[0]>0?ex.weights[0]+' ק"ג':'BW'}</div>
      </div>`).join('')}
      <button class="add-btn" onclick="openAdd('${d}')">+ הוסף תרגיל לאימון ${d}</button>`;
  }).join('');
}
...
function openEdit(id){
  ...
  document.getElementById('e-weight').value=ex.weights.join(', ');
  document.getElementById('e-coach').value=ex.coach||'';
  document.getElementById('e-rest').value=ex.rest||90;
...
function openAdd(day){
  ...
  document.getElementById('e-weight').value='20, 20, 20';
...
function saveExercise(){
  ...
  const rawW=document.getElementById('e-weight').value.split(',').map((w)=>parseFloat(w.trim())||0);
  while(rawW.length<sets) rawW.push(rawW[rawW.length-1]||0);
  const data={name,muscle:document.getElementById('e-muscle').value.trim(),day:document.getElementById('e-day').value,reps:document.getElementById('e-reps').value.trim(),sets,weights:rawW.slice(0,sets),coach:document.getElementById('e-coach').value.trim(),rest:parseInt(document.getElementById('e-rest').value)||90};
...
function deleteExercise(){
  ...
  plan=plan.filter((e)=>e.id!==editingId);
  localStorage.setItem('plan_v2',JSON.stringify(plan));
  closeModal(); renderPlan(); renderExercises(); renderTodayCard();
  showToast('🗑 נמחק');
}
